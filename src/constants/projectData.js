// src/constants/projectData.js
const projectData = [
  {
    title: "AI-Powered Email Engine",
    description: "Personalizes B2B sales emails at scale using GPT-4, LangChain, and Pinecone.",
    technologies: ["React", "Node.js", "AWS Lambda", "FastAPI"],
    link: " https://github.com/bagherianahita/AI-Powered-Sales-Email-Personalization-Engine"
  
  },
  {
    title: "LLM-Driven Market Intelligence Tool",
    description: " A market insights generator for a fintech startup that analyzes competitor press releases in near real-time.",
    technologies: [ " OpenAI GPT-4, LangChain, Pinecone, and Streamlit"],
    link: "https://github.com/bagherianahita/LLM-Market-Intelligence.git"
  }
];

export default projectData;
